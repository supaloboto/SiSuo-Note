...
<template>
  <!-- 脚本区域 -->
  <div style="position:fixed;width: 48%;">
    <h2>脚本</h2>
    <codemirror ref="inputRef" v-model="input"
                :style="{ height: '800px' }"
                :autofocus="true"
  ...
  <!-- 分割线 -->
  <div style="position:fixed;float: left; margin-left: 49%;width: 1px; height: 100vh; background-color: #d0d0d0;"></div>
  <!-- 运行结果区域 -->
  <div style="width: 48%;margin-left:50%">
    <h2>字符串分割结果</h2>
    <codemirror ref="splitResultRef" v-model="splitResult"
                :style="{ height: '200px' }"
                :autofocus="true"
                :indent-with-tab="true"
                :tab-size="2"
    ...
    <h2>AST</h2>
    <codemirror ref="astRef" v-model="ast"
                :style="{ height: '600px' }"
                :autofocus="true"
                :indent-with-tab="true"
                :tab-size="2"
                :extensions="[javascript()]"/>
    <h2>执行内容</h2>
    <codemirror ref="resultRef" v-model="result"
                :style="{ height: '600px' }"
                :autofocus="true"
                :indent-with-tab="true"
                :tab-size="2"
